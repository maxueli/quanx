...
let MT_PROVINCE = $.getdata("MT_PROVINCE") || "江苏";
let MT_CITY = $.getdata("MT_CITY") || "徐州";
...
let MT_USERAGENT =
	$.getdata("MT_USERAGENT") || "iOS;16.1.2;Apple;?unrecognized?";
let MT_R =
	$.getdata("MT_R") || "clips_OlU6TmFRag5rCXwbNAQ/Tz1SKlN8THcecBp/HGhHdw==";

if ($.isNode()) {
	const token =
		"458654D4-22D5-45E5-8B14-B762D3E4ECE8,eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpc3MiOiJtdCIsImV4cCI6MTY5OTM1MTA1MywidXNlcklkIjoxMDg4NzM0ODY4LCJkZXZpY2VJZCI6IjQ1ODY1NEQ0LTIyRDUtNDVFNS04QjE0LUI3NjJEM0U0RUNFOCIsImlhdCI6MTY5Njc1OTA1M30.rq_IoSqXAJiAca5r7BQx5wsND7-zOAB8NsXd-o3mIhQ";
	...
	MT_ITEM_BLACK = process.env.MT_ITEM_BLACK
		? process.env.MT_ITEM_BLACK
		: MT_ITEM_BLACK;
	MT_TOKENS = token;
	MT_VERSION = process.env.MT_VERSION ? process.env.MT_VERSION : MT_VERSION;
	MT_USERAGENT = process.env.MT_USERAGENT
		? process.env.MT_USERAGENT
		: MT_USERAGENT;
...
!(async () => {
	...
	function GetCookie() {
		if ($request && $request.headers) {
			if (
				($request.headers["MT-Token"] && $request.headers["MT-Device-ID"]) ||
				($request.headers["mt-token"] && $request.headers["mt-device-id"])
			) {
				let new_MT_Token =
					$request.headers["MT-Token"] || $request.headers["mt-token"];
				let new_Device_ID =
					$request.headers["MT-Device-ID"] || $request.headers["mt-device-id"];
			...
			if (
				$request.headers["MT-APP-Version"] ||
				$request.headers["mt-app-version"]
			) {
				$.MT_VERSION =
					$request.headers["MT-APP-Version"] ||
	...
	for (let i = 0; i < CookieArr.length; i++) {
		...
		if ($.userName) {
			for (const itemID of $.itemCodes) {
				...
				if (!$.shopIds) {
					let msg = `❌ ${getProductInfo($.itemId, "title")} [${
						$.itemId
					}]暂无可申购门店。\n`;
					console.log(msg);
					message += msg;
				...
				if ($.actParam) {
					console.log(
						`开始申购: ${getProductInfo($.itemId, "title")} [${$.shopId}-${
							$.itemId
						}]\n`
					);
					await $.wait(1000 * 5);
...
function getUserInfo() {
	let opt = {
		...
		headers: {
			...
			"MT-Request-ID": `${
				Date.now() * 100000 + parseInt(10000 * Math.random())
			}`,
				...
				if (data) {
					...
					if (data?.code === 2000) {
						...
						if (itemList.length > 0) {
							if (MT_ITEM_BLACK)
								console.log(`申购商品黑名单: ${MT_ITEM_BLACK}`);
...
function getShopInfo() {
	...
	let opt = {
		url: `https://static.moutai519.com.cn/mt-backend/xhr/front/mall/shop/list/slim/v3/${
			$.sessionId
		}/${encodeURIComponent(MT_PROVINCE)}/10056/${zeroDate}`,
		headers: {
					...
					if (data?.code === 2000) {
						...
						for (const _ in shops) {
							...
							if ($.shopMap[shopId]["cityName"] === MT_CITY) {
								if (
									MT_DISTRICT &&
									!new RegExp(MT_DISTRICT).test(
										$.shopMap[shopId]["districtName"]
									)
								)
									continue;
								for (const _ in items) {
									...
									console.log(
										`【${ownerName}】[${shopId}-${itemId}] ${getProductInfo(
											itemId,
											"title"
										)}  价格:${getProductInfo(
											itemId,
			...
			try {
				...
				if (err) {
					...
					if (resp.statusCode === 480) {
						result = `❌ ${getProductInfo($.itemId, "title")} 申购失败：${
					...
					if (data) {
						...
						data = JSON.parse(data);
						if (data?.code === 2000) {
							result = `🎉 ${getProductInfo($.itemId, "title")} 申购成功 - ${
								$.shopMap[$.shopId]["name"]
							}\n`;
