...
var headerscommon = {
	...
	"User-Agent":
		"Rajax/1 Apple/iPhone11,8 iOS/13.3 Eleme/8.29.6 ID/BFA5A018-7070-4341-9DEF-763E3B23EA282; IsJailbroken/1 Mozilla/5.0 (iPhone; CPU iPhone OS 13_3 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 AliApp(ELMC/8.29.6) UT4Aplus/0.0.4 WindVane/8.6.0 828x1792 WK"
};
		...
		setTimeout(() => {
			try {
				...
				let body = {
					channel: "app",
					index: random(0, 3),
					longitude: 116.334716796875,
					latitude: 59.73897171020508
				};
				url = {
					url: `https://h5.ele.me/restapi/member/v2/users/`,
					headers: headerscommon,
	...
	for (var i = 0; i < turnresult.length; i++) {
		if (turnresult[i].status == 1) {
			ret =
				ret +
				"***获得：" +
				turnresult[i].prizes[0].name +
				"(" +
				turnresult[i].prizes[0].amount +
				")元🧧\n";
		}
	}
